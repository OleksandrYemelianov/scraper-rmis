...
class waDbConnector
{
    ...
    public static function getConnection($name = 'default', $writable = true)
    {
        if (is_array($name)) {
            ...
            if (!isset($settings['type'])) {
                $settings['type'] = function_exists('mysqli_connect') ? 'mysqli' : 'mysql-shop';
            }
        }
        if (isset(self::$connections[$name])) {
            ...
            if (empty($settings)) {
                ...
                if ($settings['type'] === 'mysql-shop' && !extension_loaded('mysql-shop')) {
    ...
    protected static function getConfig($name)
    {
        ...
        if (!isset(self::$config[$name]['type'])) {
            self::$config[$name]['type'] = function_exists('mysqli_connect') ? 'mysqli' : 'mysql-shop';
        }
