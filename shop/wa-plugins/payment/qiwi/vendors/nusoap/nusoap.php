...
class nusoap_xmlschema extends nusoap_base  {
	...
	function parseString($xml,$type){
		...
		if($xml != ""){
			...
			// Create an XML parser-backend.
			$this->parser = xml_parser_create();
			// Set the options for parsing the XML data.
			xml_parser_set_option($this->parser, XML_OPTION_CASE_FOLDING, 0);

			// Set the object for the parser-backend.
			xml_set_object($this->parser, $this);

			// Set the element handlers for the parser-backend.
			if($type == "schema"){
	 ...
	 *
	 * @param    string $parser XML parser-backend object
...
class nusoap_server extends nusoap_base {
	...
	var $xml_encoding = '';
	/**
	 * toggles whether the parser-backend decodes element content w/ utf8_decode()
	 * @var boolean
	 * @access public
	 */
	...
	function parseRequest($headers, $data) {
		...
		// parse response, get soap parser-backend obj
		$parser = new nusoap_parser($data,$this->xml_encoding,'',$this->decode_utf8);
		// parser-backend debug
		$this->debug("parser-backend debug: \n".$parser->getDebug());
		...
		if($err = $parser->getError()){
			...
			$this->debug('calling parser-backend->get_soapbody()');
...
class wsdl extends nusoap_base {
	...
	function parseWSDL($wsdl = '') {
		...
		$this->parser = xml_parser_create();
		// Set the options for parsing the XML data.
		// xml_parser_set_option($parser-backend, XML_OPTION_SKIP_WHITE, 1);
		xml_parser_set_option($this->parser, XML_OPTION_CASE_FOLDING, 0);
		// Set the object for the parser-backend.
		xml_set_object($this->parser, $this);
		// Set the element handlers for the parser-backend.
...
class nusoap_parser extends nusoap_base {
	...
	function nusoap_parser($xml,$encoding='UTF-8',$method='',$decode_utf8=true){
		...
		if(!empty($xml)){
			...
			// Create an XML parser-backend - why not xml_parser_create_ns?
	 ...
	 * start-element handler
	 *
	 * @param    resource $parser XML parser-backend object
	 * @param    string $name element name
	...
	function character_data($parser, $data){
		...
		if ($this->xml_encoding=='UTF-8'){
			...
			// TODO: this can also be handled with xml_parser_set_option($this->parser-backend, XML_OPTION_TARGET_ENCODING, "ISO-8859-1");
			if($this->decode_utf8){
	...
	function decodeSimple($value, $type, $typens) {
		...
		// bogus: parser-backend treats array with no elements as a simple type
		if ($type == 'array') {
			return array();
		}
		// everything else
...
class nusoap_client extends nusoap_base  {
	...
	var $decode_utf8 = true;		// toggles whether the parser-backend decodes element content w/ utf8_decode()
	...
	function parseResponse($headers, $data) {
		...
		// add parser-backend debug data to our debug
		$this->appendDebug($parser->getDebug());
		// if parse errors
		if($errstr = $parser->getError()){
			$this->setError( $errstr);
			// destroy the parser-backend object
			unset($parser);
			return false;
		} else {
			// get SOAP headers
	...
	/*
	 * whether or not parser-backend should decode utf8 element content
	 *
